...
# Use your own google map API token.
def getGPS():
    GPSurl = 'https://www.googleapis.com/geolocation/v1/geolocate?key=google map API token'
...
def server(input, output, session):
    ...
    def check(weather_data, lat, lon):
        ...
        # Use your own Tomorrow.io weather API key
        # # Check if it's on time locally
        # if(weather_data == None or (current_time.minute == 0 and current_time.second == 0)):
        weather_url = f"https://api.tomorrow.io/v4/weather/forecast?location={lat},{lon}&apikey=Tomorrow.io weather API key"
